...
import changedoc.full_retirement_calc
...
def test_retirement_0():
    year, month = changedoc.full_retirement_calc.full_retirement_age(CURRENT_YEAR)
    assert year == 67 and month == 0


def test_retirement_1():
    year, month = changedoc.full_retirement_calc.full_retirement_age(CURRENT_YEAR + 1)
    assert year == -1 and month == -1


def test_retirement_2():
    for birth_year in TEST_RETIREMENT_AGE:
        actual_year, actual_month = changedoc.full_retirement_calc.full_retirement_age(birth_year)
...
def test_retirement_date_0():
    for birth_info in TEST_RETIREMENT_DATE:
        actual_year, actual_month = changedoc.full_retirement_calc.full_retirement_date(birth_info[TRD_BIRTH_YEAR], birth_info[TRD_BIRTH_MONTH])
